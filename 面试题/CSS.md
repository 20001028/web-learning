...
- inline元素不会占一行，宽度随元素内容变化

  设置width和height属性无效

  padding生效，margin上下不生效，左右生效

- block会占一行，默认情况下block元素宽度为父元素宽度
...
- inline-block
  ...
  元素拥有block元素可设置的属性，不会占一行显示

### 清除浮动

为什么要清除浮动：如下，div中出现两个span元素后，span元素有float属性，会导致div元素高度变为0
